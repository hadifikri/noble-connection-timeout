...
Noble.prototype.cancelLeConn = function(callback) {
   if (platform === 'linux' || platform === 'win32'){
      this._bindings.cancelLeConn();
      callback();
   }
   else {
      this.emit('warning', 'cancelLeConn only supported on linux/win32');
      callback("error: cancelLEConn only supported on linux/win32");
   }
};
